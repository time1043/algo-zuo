...
class FindNumberTest {
    ...
    TestCase generateArray(int maxSize, int maxValue) {
        ...
        while (set.size() < size)
            set.add((int) (Math.random() * (maxValue * 2 + 1)) - maxValue);
        int[] nums = set.stream().sorted().mapToInt(Integer::intValue).toArray(); // sort
    ...
    void binarySearch() {
        for (int i = 0; i < epoch; i++) {
            ...
            int actual = FindNumber.binarySearch(nums, target);
